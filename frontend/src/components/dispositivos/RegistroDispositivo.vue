...
<script>
import api from '@/api/api'; // Importa el archivo de configuración de Axios
  ...
  methods: {
    async guardar() {
      try {
        ...
        // Actualizar el estado de Vuex con la respuesta del backend
        this.$store.commit('setDispositivos', [...this.$store.state.dispositivos, response.data]);
        this.limpiar();
      } catch (error) {
        this.alerta.mensaje = 'Error al guardar el dispositivo: ' + error.message;
      }
    },
    limpiar() {
      this.dispositivo = { /* misma estructura inicial que en data */ };
